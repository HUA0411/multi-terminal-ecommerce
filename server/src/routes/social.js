...
router.post("/products/:id/reviews", auth(), asyncHandler(async (req, res) => {
  ...
  // 校验购买记录：订单已完成且含该商品
  const bought = store.find("orderItems", (i) => i.productId === product.id && i.orderId && store.get("orders", i.orderId)?.userId === req.user.id && store.get("orders", i.orderId)?.status === "completed");
  if (!bought.length) return fail(403, 403, "仅已完成的购买订单可评价");
  const review = store.insert("reviews", { productId: product.id, userId: req.user.id, nickname: req.user.nickname, avatar: req.user.avatar, rating, content, status: "active", createdAt: now() });
  // 更新商品评分（最近评价平均）
