...
import { auth, rateLimit } from "../../_shared/middleware.js";
import { callInternal } from "../../_shared/internal-client.js";
import { asyncHandler, ok, fail, hashPassword, verifyPassword, signToken, signRefresh, verifyToken, publicUser, uid } from "../../_shared/util.js";
...
router.post("/register", rateLimit({ max: 5, name: "register" }), asyncHandler(async (req, res) => {
  ...
  if (inviteCode) {
    try {
      const inv = await callInternal("marketing", "GET", "/internal/shares/find", null, { code: inviteCode });
...
router.post("/login", rateLimit({ max: 20, name: "login" }), asyncHandler(async (req, res) => {
  ...
  if (!user || !verifyPassword(password || "", user.password)) {
    callInternal("platform", "POST", "/internal/risk-events", { userId: user ? user.id : null, type: "login_fail", level: "low", detail: { reason: "账号或密码错误" }, ip: req.ip, createdAt: new Date().toISOString() }).catch(() => {});
