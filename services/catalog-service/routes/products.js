...
import { cache, optionalAuth } from "../../_shared/middleware.js";
import { asyncHandler, ok, fail, paginate } from "../../_shared/util.js";
import { serializeProduct, convert, ensureMerchants, merchantMap } from "./common.js";
...
router.get("/:id", optionalAuth, asyncHandler(async (req, res) => {
  ...
  await ensureMerchants();
  const merchant = merchantOf(p.merchantId);
...
function merchantOf(id) {
  const m = merchantMap()[id];
  return m ? { id: m.id, name: m.name, logo: m.logo, rating: m.rating, description: m.description } : null;
}
